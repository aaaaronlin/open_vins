...
void Loader::load_data(std::string path_traj,
    ...
    while(std::getline(file, current_line)) {
        ...
        // Loop through this line (timestamp(s) tx ty tz vx vy vz qx qy qz qw Pr11 Pr12 Pr13 Pr22 Pr23 Pr33 Pt11 Pt12 Pt13 Pt22 Pt23 Pt33)
        while(std::getline(s,field,' ')) {
            // Skip if empty
            if(field.empty() || i >= data.rows())
                continue;
        ...
        if(i >= 20) {
        	...
        	poses.push_back(pos);
            vel.push_back(data.block(4,0,3,1));
            // covariance values
            Eigen::Matrix3d c_ori, c_pos;
